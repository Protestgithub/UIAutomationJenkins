...
And('Enter all the Stock Information', function () {
  ...
  stockInitiationPage.getToStockAccount().click()
  stockInitiationPage.getTrustBank().select("TBN101")
  stockInitiationPage.getSystemErrorMessage().contains("X`XTo: System Account is required")
...
When('Navigate to Admin Transaction and Click on More', function () {
  cy.intercept('/mobiquitypay/v1/service-config*').as('getAdminTxn')
stockInitiationPage.getAdminTransaction().click()
 cy.wait('@getAdminTxn', {timeout:20000})
cy.intercept('/mobiquitypay/v1/service-config*').as('getAllocation')
stockInitiationPage.getMoreView().click({force:true})
cy.wait(3000)
})
And('Assert all the fields of Admin Transaction',function(){
  stockInitiationPage.getAssertAllField().should('contain','Stock Creation')
                                         .should('contain','Stock Withdraw')
                                         .should('contain','Stock Transfer')
                                         .should('contain','Allocation')
                                         .should('contain','Reimbursement')
                                         .should('contain','Stock Balance Enquiry')
                                         .should('contain','Bank Balance Enquiry')
})
And('Assert Stock Creation',function(){
  stockInitiationPage.getAssertStockPage().should('contain','Enter stock information')
  stockInitiationPage.getAssertCurrency().should('contain','Currency')
  stockInitiationPage.getAssertTrustBank().should('contain','From: Trust Bank')
})
