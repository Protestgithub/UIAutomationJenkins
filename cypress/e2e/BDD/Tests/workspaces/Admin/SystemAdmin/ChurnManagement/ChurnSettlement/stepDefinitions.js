...
Then('Click on Initiated settlement data and approve',function(){
  ...
  cy.wait(2000)
  churnManagementPage.getChurnInitiationMessage().then((para)=>{
  let a = para.text()
  let b = a.split(':')
  let c = b[1]
  cy.readFile(churnSubRegistration).then((d) => {
    d.ChurnTransactionID = c
    cy.writeFile(churnSubRegistration, d)
  })
})
})
Then('Verify the churn settlement Transaction',function(){
  cy.readFile(churnSubRegistration).then((data) => {
    let TID = data.ChurnTransactionID
    manageUsersPage.getSearchTransactionId().type(TID, { force: true }).click({ force: true })
    manageUsersPage.getChurnTransactionID().eq(0).should('have.text',TID)
  })
})
