...
//-------------------------------------------------
getCurrentTime(){
    return cy.iframe().find('[class="xdsoft_time xdsoft_current"]')
}
getStartTime(){
    return cy.iframe().find('[data-hour="0"]').eq(0)
}
getEndTime(){
    return cy.iframe().find('[data-hour="23"]').eq(1)
}
getDeleteAllRules(){
    return cy.iframe().find('[class="material-icons small remove-charge-rule modal-trigger float"]')
}
getGoldCustomer(){
    return cy.iframe().find('[data-value="SUBS"]')
}
getSilverCustomer(){
    return cy.iframe().find('[data-value="SSUBS"]')
}
getInActive(){
    return cy.iframe().find('[class="switch-label"]')
}
getTime(){
    return cy.iframe().find('[class="xdsoft_time_box xdsoft_scroller_box"]')
}
}
