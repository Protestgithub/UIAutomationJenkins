class SendMoney{
    getMobileNumber(){
        return cy.get('[class="w-11/12 mx-4 ng-untouched ng-pristine ng-invalid"]')
    }
...
getSelectNormalWallet(){
    return cy.get(':nth-child(2) > .h-10 > .w-44')
}
getProceedBtn(){
    return cy.get('[data-test-id="send-money-proceed-button"]')
}
getATMInfoHeading(){
...
getCopyTransactionId(){
    return cy.get('[class="text-center inter text-sm font-light text-gray-400"]')
}
getHistory(){
